    ...
    while running:          
        ...
        if current_movement == 'stop':
            ...
            if in_braking_phase and USE_VELOCITY_BRAKING:
                # Calculate wheel velocities in ticks/s
                ...
                # Calculate acceleration (derivative of velocity) in ticks/s²
                ...
                if current_time - last_debug_print_time >= DEBUG_PRINT_INTERVAL:
                    if in_braking_phase and USE_VELOCITY_BRAKING:
                        print(f"Braking - Left: v={left_velocity:.1f}ticks/s pwm={left_brake_pwm:.1f}, "
                            f"Right: v={right_velocity:.1f}ticks/s pwm={right_brake_pwm:.1f}")
